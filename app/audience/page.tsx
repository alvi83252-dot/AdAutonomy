            ...
            {c.audiences.map((channel, i) => (
              <AnimatedCard key={channel.name} delay={i * 0.1}>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs px-2 py-1 bg-primary/15 text-primary rounded">{channel.platform}</span>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div>
                    <p className="text-2xl font-bold tabular-nums text-primary">{channel.reach.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground">Est. Reach</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold tabular-nums text-foreground">${channel.costPerClick}</p>
